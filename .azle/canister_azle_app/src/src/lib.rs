...
fn stable_b_tree_map_contains_key(
    ...
    match memory_id {
        0u8 => {
            let wrapped_key = &StableBTreeMap0KeyType(
...
struct StableBTreeMap0ValueType(MedicalAppointment);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
#[ic_cdk_macros::query(name = "searchAppointments")]
#[candid::candid_method(query, rename = "searchAppointments")]
async fn _cdk_user_defined_searchAppointments(
    _cdk_user_defined_query: String,
) -> (_AzleResult<Vec<MedicalAppointment>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "searchAppointments".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("searchAppointments", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "searchAppointments"),
                    ))?;
...
#[ic_cdk_macros::query(name = "getAppointments")]
#[candid::candid_method(query, rename = "getAppointments")]
async fn _cdk_user_defined_getAppointments() -> (_AzleResult<
    Vec<MedicalAppointment>,
    String,
>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getAppointments".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getAppointments", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getAppointments"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getAppointment")]
#[candid::candid_method(query, rename = "getAppointment")]
async fn _cdk_user_defined_getAppointment(
    _cdk_user_defined_id: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::query(name = "getUpcomingAppointments")]
#[candid::candid_method(query, rename = "getUpcomingAppointments")]
async fn _cdk_user_defined_getUpcomingAppointments(
    _cdk_user_defined_daysAhead: _CdkFloat64,
) -> (_AzleResult<Vec<MedicalAppointment>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getUpcomingAppointments".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getUpcomingAppointments", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getUpcomingAppointments"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_daysAhead
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::query(name = "getCanceledAppointments")]
#[candid::candid_method(query, rename = "getCanceledAppointments")]
async fn _cdk_user_defined_getCanceledAppointments() -> (_AzleResult<
    Vec<MedicalAppointment>,
    String,
>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getCanceledAppointments".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getCanceledAppointments", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getCanceledAppointments"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::update(name = "scheduleAppointment")]
#[candid::candid_method(update, rename = "scheduleAppointment")]
async fn _cdk_user_defined_scheduleAppointment(
    _cdk_user_defined_appointment: MedicalAppointment,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "scheduleAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("scheduleAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "scheduleAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_appointment
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::update(name = "cancelAppointment")]
#[candid::candid_method(update, rename = "cancelAppointment")]
async fn _cdk_user_defined_cancelAppointment(
    _cdk_user_defined_id: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "cancelAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("cancelAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "cancelAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "sendReminder")]
#[candid::candid_method(update, rename = "sendReminder")]
async fn _cdk_user_defined_sendReminder(
    _cdk_user_defined_id: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "sendReminder".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("sendReminder", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "sendReminder"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateAppointment")]
#[candid::candid_method(update, rename = "updateAppointment")]
async fn _cdk_user_defined_updateAppointment(
    _cdk_user_defined_id: String,
    _cdk_user_defined_updatedAppointment: MedicalAppointment,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_id.try_into_vm_value(&mut boa_context)?,
                            _cdk_user_defined_updatedAppointment
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::update(name = "deleteAppointment")]
#[candid::candid_method(update, rename = "deleteAppointment")]
async fn _cdk_user_defined_deleteAppointment(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Option<MedicalAppointment>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "deleteAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("deleteAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "deleteAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
async fn _cdk_user_defined_updateMedicalRecord(
    _cdk_user_defined_id: String,
    _cdk_user_defined_newRecord: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
...
#[ic_cdk_macros::update(name = "rescheduleAppointment")]
#[candid::candid_method(update, rename = "rescheduleAppointment")]
async fn _cdk_user_defined_rescheduleAppointment(
    _cdk_user_defined_id: String,
    _cdk_user_defined_newDate: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "rescheduleAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("rescheduleAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "rescheduleAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "completeAppointment")]
#[candid::candid_method(update, rename = "completeAppointment")]
async fn _cdk_user_defined_completeAppointment(
    _cdk_user_defined_id: String,
) -> (_AzleResult<MedicalAppointment, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "completeAppointment".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("completeAppointment", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "completeAppointment"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
struct MedicalAppointment {
    id: Box<String>,
    patientName: Box<String>,
    doctorName: Box<String>,
    appointmentDate: Box<String>,
    isScheduled: Box<bool>,
    reminderSent: Box<bool>,
    medicalRecord: Box<String>,
    createdAt: Box<Option<u64>>,
